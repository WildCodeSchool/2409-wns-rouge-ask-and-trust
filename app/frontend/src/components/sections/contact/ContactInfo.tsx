...
import { contactMethods, supportOptions } from "@/components/sections/contact/mockContact"
            ...
            <div key={index} className="flex items-start space-x-3">
              ...
              <div className="flex-1">
                ...
                {method.action ? (
                  ...
                  <p className="text-sm whitespace-pre-line">{method.value}</p>
                )}
              </div>
            </div>
          ))}
        ...
        <CardContent className="space-y-4">
          {supportOptions.map((option, index) => (
            <div key={index} className="flex items-center justify-between rounded-lg border p-3">
              ...
              <Button size="sm" variant="outline" ariaLabel={option.title}>
                {option.action.includes("@") ? (
                  <a href={`mailto:${option.action}`}>
                    {option.action.includes("@") ? "Écrire" : option.action}
                  </a>
                ) : (
                  option.action
                )}
