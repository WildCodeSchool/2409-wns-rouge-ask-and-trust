    ...
    return (
        <header lang="fr" className="flex items-center justify-between gap-10 px-5 pt-4 mb-20" role="contentinfo" aria-label="En-tête de page">
            ...
            {isMobile ? (
                ...
                <>
                    <NavAndAuthButtons headerLinks={HEADER_LINKS} isMobile={false} />
                </>
            )}
        </header>
    )
...
function HeaderLink({ href, label, category, ariaLabel }: LinksType) {
    ...
    return (
        <Link
            to={href}
            className="text-primary-700 font-bold max-lg:transition-padding max-lg:duration-200 max-lg:ease-in-out max-lg:hover:pl-5"
