...
export default function Footer() {
  return (
    <footer lang="fr" className="relative w-full bg-primary-700 pt-16" role="contentinfo" aria-label="Pied de page">
      {/* Curved top edge - decorative element */}
      <div className="absolute top-0 left-0 w-full overflow-hidden" style={{ height: "50px" }} aria-hidden="true">
        <svg
        ...
        >
          <path d="M0 0H1440V100C1440 100 1082.5 0 720 0C357.5 0 0 100 0 100V0Z" fill="white" />
        </svg>
      </div>

      <div className="w-full px-4 pb-8" role="navigation" aria-label="Plan du site">
        <div className="flex flex-col md:flex-row md:justify-between items-center md:items-end">
          ...
          <nav className={cn("md:text-left text-center md:order-1 order-2 mb-8 md:mb-0", "flex flex-col space-y-3")} aria-label="Navigation du pied de page">
            ...
            <div className="md:hidden mt-6" role="separator">
              <Separator className="bg-black-300" />
            </div>

            {/* Copyright */}
            <div className="mt-8 text-center md:text-left md:mt-4">
              <p className="md:flex md:flex-wrap md:items-center md:space-x-2 flex flex-col md:flex-row text-sm text-primary-50">
          ...
          {/* Logo desktop */}
          <div className="hidden md:block md:order-2 mr-2" aria-hidden="true">
            <div className="relative flex items-center">
              <img src={logoFooter} alt="Logo AskTrust" />
            </div>
          </div>
 ...
 */
function FooterLink({ href, label, category, ariaLabel }: FooterLinkType) {
